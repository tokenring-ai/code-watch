...
export default class CodeWatchService extends Service {
 ...
 async processFileForAIComments(filePath) {
  try {
   ...
   console.error(`Error processing file ${filePath} for AI comments:`, error);
  }
 }
 ...
 async checkAndTriggerAIAction(line, filePath, lineNumber) {
  ...
  else if (line.endsWith('AI') || line.endsWith('AI!') || line.endsWith('AI?')) {
   await this.handleAIComment(line, filePath, lineNumber);
  }
 }
 ...
 async triggerCodeModification(content, filePath, lineNumber) {
  ...
  const modelRegistry = this.registry.requireFirstServiceByType(ModelRegistry);

  chatService.systemLine(`[CodeWatchService] Code modification triggered from ${filePath}:${lineNumber}`);
  ...
  const systemPrompt = {
   role: "system",
   content: "When you output a file with file tool, you MUST remove any lines that end with AI!. It is a critical failure to leave these lines in the file."
  };
...
${fileContent}`.trim()}
  ...
  const client = await modelRegistry.chat.getFirstOnlineClient("auto:intelligence>=3,tools>=2");

  chatService.systemLine(`[CodeWatchService] Using model ${client.getModelId()}`);

  const request = await createChatRequest({input, systemPrompt}, this.registry);
 ...
 async triggerQuestionAnswer(content, filePath, lineNumber) {
  const chatService = this.registry.requireFirstServiceByType(ChatService);
  chatService.infoLine(`[CodeWatchService][AI?] Question answering noted from ${filePath}:${lineNumber}. This feature is not fully implemented yet.`);
 ...
 async noteAIComment(content, filePath, lineNumber) {
  const chatService = this.registry.requireFirstServiceByType(ChatService);
  chatService.infoLine(`[CodeWatchService][AI] AI comment noted from ${filePath}:${lineNumber}. This feature is not fully implemented yet.`);
